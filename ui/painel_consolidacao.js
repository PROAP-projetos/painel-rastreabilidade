...
function normalizarPropostaPlana(p) {
    ...
    const unidadesCampo = Array.isArray(p && p.unidades) ? p.unidades.filter(Boolean) : (p && p.unidade ? [p.unidade] : []);
...
function construirConsolidadasDePropostas(propostas) {
    ...
    (Array.isArray(propostas) ? propostas : []).forEach(p => {
        ...
        if (!mapa.has(key)) {
            mapa.set(key, { id, acao: acaoTexto, unidade: '', unidades: [], propostas: [] });
        }
...
function deduplicarConsolidadas(lista) {
    ...
    (lista || []).forEach(item => {
        ...
        if (!mapa.has(key)) {
            ...
            mapa.set(key, { ...item, unidades, propostas: Array.isArray(item.propostas) ? [...item.propostas] : [] });
            return;
        }
...
// ── Helpers de marcadores (CORRIGIDO PARA TELEPORTE) ────────────────────
